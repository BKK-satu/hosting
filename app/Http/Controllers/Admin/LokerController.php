...
class LokerController extends Controller
{
    ...
    */
    public function index(Request $request)
    {
        ...
        $search = $request->q;
        if ($search) {
            $loker = Lowongankerja::orderBy('created_at', 'DESC')->orderBy('status', 'ASC')->where('title','like',"%".$search."%")
                ->orWhere('posisi','like',"%".$search."%")
                ->orWhere('kategori','like',"%".$search."%")
                ->paginate(10);
            $searchData = $request->q;
        }else{
            $loker = Lowongankerja::orderBy('status', 'ASC')->orderBy('created_at', 'ASC')->paginate(10);
            $searchData = '';
        }
        ...
        foreach ($mitra as $item) {
            if ($item->foto === 'default-company.png') {
